...
export class SceneManager {
	...
	parseScene(sceneText: string, options: ParseSceneOptions = {}): SceneGraph {
		...
		if (!this.validateScene(document)) {
			const issues = (this.validateScene.errors ?? []).map((err: ErrorObject) => {
				const pointer = 'instancePath' in err ? (err as ErrorObject & { instancePath?: string }).instancePath ?? '/' : err.schemaPath;
	...
	private instantiateNode(
		...
		if (index.has(definition.id)) {
			throw new SceneValidationError(`Duplicate node id "${definition.id}" detected.`, [sceneIdentifier]);
		}
	...
	private createNodeFromDefinition(definition: SceneNodeDefinition, parent: NodeBase | null): NodeBase {
		...
		switch (definition.type) {
			case 'Sprite2D': {
				const { position, scale, rotation, texturePath, ...rest } = baseProps.properties as Record<string, unknown>;
				return new Sprite2D(
					{
						...baseProps,
						properties: rest,
			...
			case 'Node3D':
			case undefined: {
				const { position, rotation, scale, ...restProps } = baseProps.properties as Record<string, unknown>;
				return new Node3D(
					{
						...baseProps,
