...
export class OperationService {
	...
	async undo(): Promise<boolean> {
		...
		try {
			...
			if (undone) {
				...
				const nextUndoEntry =
					nextSnapshot.undoEntries[nextSnapshot.undoEntries.length - 1] ?? null;
				this.state.operations.lastUndoableCommandId =
					nextUndoEntry?.metadata.commandId ?? null;
	...
	private async executeOperation<
		TInvokeResult extends OperationInvokeResult,
	>(
		operation: Operation<TInvokeResult>,
		options: OperationInvokeOptions,
