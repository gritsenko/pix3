...
export class FileSystemAPIService {
	...
	async requestProjectDirectory(mode: PermissionMode = 'readwrite'): Promise<FileSystemDirectoryHandle> {
		try {
	...
	async ensurePermission(
		...
		if (!permissionHandle.queryPermission || !permissionHandle.requestPermission) {
			throw new FileSystemAPIError('unsupported', 'Permission APIs are unavailable in this environment.');
		}
		...
		if (requestResult !== 'granted') {
			throw new FileSystemAPIError('permission-denied', 'Permission denied for requested operation.');
		}
	}
	...
	async getFileHandle(path: string, options?: { mode?: PermissionMode }): Promise<FileSystemFileHandle> {
	...
	private async resolveFileHandle(path: string, mode: PermissionMode): Promise<FileSystemFileHandle> {
